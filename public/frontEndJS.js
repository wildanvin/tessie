...
;(() => {
    ...
    fetch('/engraveSlot1', {
      ...
      .then((res) => {
        if (res.ok) {
          ...
          alert(`The following text has been generated: 
                ${line1M1.value}
                ${line2M1.value}`)
        } else {
          console.log('Text is too long')
          alert(`Text is too large, try with a small one or another tag`)
        }
      })
  ...
  engraveButtonM1.onclick = () => {
    alert(`You are going to engrave the following text: 
            ${line1M1.value}
            ${line2M1.value}`)
